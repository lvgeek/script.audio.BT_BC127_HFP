...
# 
# May 2015- Dave Freeman, lvengineering adapted for RPi use
# 
...
BC127::opResult BC127::BLEScan(timeout)
  ...
  unsigned loopTimeout = timeout*1300
  ...
  while loopStart + loopTimeout > millis():
    ...
    if _serialPort.available() >0) buffer.concat(char(_serialPort.os.read()):
...
#  response. 
BC127::opResult BC127::exitDataMode(guardDelay)
  String buffer
...
BC127::opResult BC127::connect(index, connType connection)
...
BC127::opResult BC127::inquiry(timeout)
    ...
    if buffer.endsWith(EOL):
      ...
      if (buffer.startsWith("IN")) # An address has been found!
      {
             ...
             #  the list and the list isn't too long.
        {
          ...
          for i in range(_numAddresses+1):
            if addressTemp == _addresses[i]:
              addressTemp = "x"
              break
...
#  requested index.
BC127::opResult BC127::getAddress(index, String &address)
...
BC127::opResult BC127::connectionState()
  ...
  #  the buffer.
  while _serialPort.available() > 0) _serialPort.os.read(:
  return retVal
